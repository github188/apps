...
static int i2c_diskpw_initialized;
static int i2c_diskpw_fd;
static int i2c_fd;
...
int do_read_diskpw(uint8_t reg1, uint8_t reg2)
{
	...
	while(count--) {
		ret = i2c_smbus_read_byte_data(i2c_diskpw_fd ,reg1);
		if (ret >= 0)
			return ret;
	}
	return PERR_IOERR;
...
int i2c_read_diskpw(void)
{
	...
	if (ioctl(i2c_diskpw_fd, I2C_SLAVE_FORCE, I2C_ADDRESS_DISKPW1) < 0) {
		return PERR_NODEV;
	}
	reg1 = I2C_GP1_MODE1;
	reg2 = I2C_GP1_MODE2;
	...
	if (ioctl(i2c_diskpw_fd, I2C_SLAVE_FORCE, I2C_ADDRESS_DISKPW2) < 0) {
		return PERR_NODEV;
	}
	reg1 = I2C_GP1_MODE1;
	reg2 = I2C_GP1_MODE2;
...
int do_write_diskpw(int mode, uint8_t reg1, uint8_t reg2)
{
	...
	while(count--) {
		value = i2c_smbus_read_byte_data(i2c_diskpw_fd, reg2);
		if (value == -1)
			continue;
		value &= 0xf0;
		ret = i2c_smbus_write_byte_data(i2c_diskpw_fd, reg2, value);
		if (ret == -1)
			continue;
	
		ret = i2c_smbus_write_byte_data(i2c_diskpw_fd ,reg1, mode);
		if (ret == 0)
			return PERR_SUCCESS;
	}
	return PERR_IOERR;
